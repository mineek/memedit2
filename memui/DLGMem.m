...
void search_progress_callback(int current, int total, void *user_data) {
    DLGMem *self = (__bridge DLGMem *)user_data;
    float progress = (float)current / (float)total;
    [self.memView updateSearchProgress:progress];
}
...
- (void)searchMem:(const char *)value type:(int)type comparison:(int)comparison {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        int found = 0;
        search_result_chain_t chain = self->g_chain;
        search_result_chain_t new_chain = search_mem_with_progress(self->g_task, v, size, type, comparison, chain, &found, search_progress_callback, (__bridge void *)self);
        dispatch_async(dispatch_get_main_queue(), ^{
            self->g_chain = new_chain;
            self.memView.chainCount = found;
            self.memView.chain = self->g_chain;
            [self.memView hideSearchProgress];
        });
    });
}

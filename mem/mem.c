...
typedef struct {
    ...
    void *user_data;
} thread_search_context_t;

void free_memory_regions(memory_region_t *head) {
    while (head) {
        memory_region_t *next = head->next;
        free(head);
        head = next;
    }
}
...
void *search_thread_worker(void *arg) {
    thread_search_context_t *ctx = (thread_search_context_t *)arg;
    search_result_chain_t local_chain = NULL;
    search_result_chain_t chain_tail = NULL;

    memory_region_t *region = ctx->regions;
    while (region) {
        mach_vm_offset_t address = region->address;
        mach_vm_size_t region_size = region->size;
        vm_prot_t protection = region->protection;

        void *data = malloc(region_size);
        mach_vm_size_t data_size = 0;

        kern_return_t kret_read = mach_vm_read_overwrite(ctx->task, address, region_size, (mach_vm_address_t)data, &data_size);
        if (kret_read == KERN_SUCCESS) {
            void *big = data;
            ...
            do {
                result = search_mem_value(big, big_size, ctx->value, ctx->value_size, ctx->value_type, ctx->comparison);
                ...
                big = result + ctx->value_size;
                big_size = data_size - result_address_slide - ctx->value_size;

                search_result_chain_t c = create_search_result_chain(result_address, ctx->value, ctx->value_size, ctx->value_type, protection);
                if (local_chain == NULL) {
                    local_chain = c;
                    chain_tail = c;
                } else {
            ...
            } while (big_size >= ctx->value_size);
        }
        free(data);

        // progress
        if (ctx->progress_callback && ctx->progress_mutex) {
            pthread_mutex_lock(ctx->progress_mutex);
            (*ctx->progress_counter)++;
            int current = *ctx->progress_counter;
            pthread_mutex_unlock(ctx->progress_mutex);

            ctx->progress_callback(current, ctx->total_regions, ctx->user_data);
        }
    ...
    if (local_chain && ctx->result_mutex) {
        pthread_mutex_lock(ctx->result_mutex);
        if (ctx->results == NULL) {
            ctx->results = local_chain;
        } else {
            // append to the end
            search_result_chain_t tail = ctx->results;
            while (tail->next) tail = tail->next;
            tail->next = local_chain;
        }
...
search_result_chain_t search_mem_first_with_progress(mach_port_t task, void *value, int size, int type, int comparison, int *length, search_progress_callback_t progress_callback, void *user_data) {
    mach_vm_offset_t address = 0;
    mach_vm_size_t region_size = 0;
    vm_region_flavor_t flavor = VM_REGION_BASIC_INFO_64;
    mach_port_t object_name = 0;
    ...
    memory_region_t *region_list_head = NULL;
    memory_region_t *region_list_tail = NULL;
    int total_regions = 0;
    ...
    // prep
    int regions_per_thread = (total_regions + NUM_SEARCH_THREADS - 1) / NUM_SEARCH_THREADS;
    pthread_t threads[NUM_SEARCH_THREADS];
    thread_search_context_t contexts[NUM_SEARCH_THREADS];
    pthread_mutex_t result_mutex = PTHREAD_MUTEX_INITIALIZER;
    pthread_mutex_t progress_mutex = PTHREAD_MUTEX_INITIALIZER;
    int progress_counter = 0;
    search_result_chain_t global_results = NULL;

    memory_region_t *current_region = region_list_head;
    int thread_count = 0;

    for (int i = 0; i < NUM_SEARCH_THREADS && current_region != NULL; i++) {
        contexts[i].task = task;
        contexts[i].value = value;
        contexts[i].value_size = size;
        contexts[i].value_type = type;
        contexts[i].comparison = comparison;
        contexts[i].regions = current_region;
        contexts[i].region_count = 0;
        contexts[i].results = NULL;
        contexts[i].result_mutex = &result_mutex;
        contexts[i].progress_mutex = &progress_mutex;
        contexts[i].progress_counter = &progress_counter;
        contexts[i].total_regions = total_regions;
        contexts[i].progress_callback = progress_callback;
        contexts[i].user_data = user_data;

        memory_region_t *thread_last_region = current_region;
        for (int j = 0; j < regions_per_thread && current_region != NULL; j++) {
            contexts[i].region_count++;
            thread_last_region = current_region;
            current_region = current_region->next;
        }

        if (thread_last_region) {
            thread_last_region->next = NULL;
        }

        pthread_create(&threads[i], NULL, search_thread_worker, &contexts[i]);
        thread_count++;

        if (current_region) {
            contexts[i + 1].regions = current_region;
        }
    }
    ...
    for (int i = 0; i < thread_count; i++) {
        ...
        if (contexts[i].results) {
            if (global_results == NULL) {
                global_results = contexts[i].results;
            } else {
                search_result_chain_t tail = global_results;
                while (tail->next) tail = tail->next;
                tail->next = contexts[i].results;
            }
        }
    }

    int count = 0;
    search_result_chain_t c = global_results;
    while (c) {
        count++;
        c = c->next;
    }

    free_memory_regions(region_list_head);
...
search_result_chain_t search_mem_first(mach_port_t task, void *value, int size, int type, int comparison, int *length) {
    return search_mem_first_with_progress(task, value, size, type, comparison, length, NULL, NULL);
}
...
search_result_chain_t search_mem_with_progress(mach_port_t task, void *value, int size, int type, int comparison, search_result_chain_t chain, int *length, search_progress_callback_t progress_callback, void *user_data) {
    search_result_chain_t result = NULL;
    if (chain == NULL) result = search_mem_first_with_progress(task, value, size, type, comparison, length, progress_callback, user_data);
    else result = search_mem_in_chain(task, value, size, type, comparison, chain, length);
    return result;
}
